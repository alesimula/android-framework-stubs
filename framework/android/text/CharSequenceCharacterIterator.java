package android.text;

public class CharSequenceCharacterIterator implements java.text.CharacterIterator {
    private final int mBeginIndex = 0;
    private final int mEndIndex = 0;
    private int mIndex;
    private final java.lang.CharSequence mCharSeq = null;
    public CharSequenceCharacterIterator(java.lang.CharSequence p0, int p1, int p2) {}
    public char first() { return 0; }
    public char last() { return 0; }
    public char current() { return 0; }
    public char next() { return 0; }
    public char previous() { return 0; }
    public char setIndex(int p0) { return 0; }
    public int getBeginIndex() { return 0; }
    public int getEndIndex() { return 0; }
    public int getIndex() { return 0; }
    public java.lang.Object clone() { return null; }
}
