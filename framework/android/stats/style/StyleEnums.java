package android.stats.style;

public final class StyleEnums {
    public static final int APP_ICON_STYLE_EXTENDIBLE_THEME = 2;
    public static final int APP_ICON_STYLE_THEMED = 1;
    public static final int APP_ICON_STYLE_UNSPECIFIED = 0;
    public static final int APP_LAUNCHED = 22;
    public static final int CLOCK_APPLIED = 33;
    public static final int CLOCK_COLOR_APPLIED = 34;
    public static final int CLOCK_SIZE_APPLIED = 35;
    public static final int CLOCK_SIZE_DYNAMIC = 1;
    public static final int CLOCK_SIZE_SMALL = 2;
    public static final int CLOCK_SIZE_UNSPECIFIED = 0;
    public static final int COLOR_PRESET_APPLIED = 26;
    public static final int COLOR_SOURCE_HOME_SCREEN_WALLPAPER = 1;
    public static final int COLOR_SOURCE_LOCK_SCREEN_WALLPAPER = 2;
    public static final int COLOR_SOURCE_PRESET_COLOR = 3;
    public static final int COLOR_SOURCE_UNSPECIFIED = 0;
    public static final int COLOR_WALLPAPER_HOME_APPLIED = 23;
    public static final int COLOR_WALLPAPER_HOME_LOCK_APPLIED = 25;
    public static final int COLOR_WALLPAPER_LOCK_APPLIED = 24;
    public static final int CURATED_PHOTOS_FETCH_END = 44;
    public static final int CURATED_PHOTOS_FETCH_START = 43;
    public static final int CURATED_PHOTOS_RENDER_COMPLETE = 45;
    public static final int DARK_THEME_APPLIED = 39;
    public static final int DATE_MANUAL = 2;
    public static final int DATE_PREFERENCE_UNSPECIFIED = 0;
    public static final int DATE_UNAVAILABLE = 1;
    public static final int DEFAULT_ACTION = 0;
    public static final int EFFECT_APPLIED_ABORTED = 4;
    public static final int EFFECT_APPLIED_OFF = 3;
    public static final int EFFECT_APPLIED_ON_FAILED = 2;
    public static final int EFFECT_APPLIED_ON_SUCCESS = 1;
    public static final int EFFECT_APPLIED_STARTED = 5;
    public static final int EFFECT_PREFERENCE_UNSPECIFIED = 0;
    public static final int ENTER_EXTENDIBLE_THEME_ICON_PICKER = 46;
    public static final int ENTER_SCREEN = 42;
    public static final int GRID_APPLIED = 32;
    public static final int LAUNCHED_CROP_AND_SET_ACTION = 6;
    public static final int LAUNCHED_DEEP_LINK = 7;
    public static final int LAUNCHED_KEYGUARD = 9;
    public static final int LAUNCHED_LAUNCHER = 1;
    public static final int LAUNCHED_LAUNCH_ICON = 5;
    public static final int LAUNCHED_PREFERENCE_UNSPECIFIED = 0;
    public static final int LAUNCHED_SETTINGS = 2;
    public static final int LAUNCHED_SETTINGS_SEARCH = 8;
    public static final int LAUNCHED_SUW = 3;
    public static final int LAUNCHED_TIPS = 4;
    public static final int LIVE_WALLPAPER_APPLIED = 16;
    public static final int LIVE_WALLPAPER_CUSTOMIZE_SELECT = 18;
    public static final int LIVE_WALLPAPER_DELETE_FAILED = 15;
    public static final int LIVE_WALLPAPER_DELETE_SUCCESS = 14;
    public static final int LIVE_WALLPAPER_DOWNLOAD_CANCELLED = 13;
    public static final int LIVE_WALLPAPER_DOWNLOAD_FAILED = 12;
    public static final int LIVE_WALLPAPER_DOWNLOAD_SUCCESS = 11;
    public static final int LIVE_WALLPAPER_EFFECT_SHOW = 21;
    public static final int LIVE_WALLPAPER_INFO_SELECT = 17;
    public static final int LIVE_WALLPAPER_QUESTIONNAIRE_APPLIED = 20;
    public static final int LIVE_WALLPAPER_QUESTIONNAIRE_SELECT = 19;
    public static final int LOCATION_CURRENT = 2;
    public static final int LOCATION_MANUAL = 3;
    public static final int LOCATION_PREFERENCE_UNSPECIFIED = 0;
    public static final int LOCATION_UNAVAILABLE = 1;
    public static final int LOCK_SCREEN_NOTIFICATION_APPLIED = 37;
    public static final int ONRESUME = 1;
    public static final int ONSTOP = 2;
    public static final int PICKER_APPLIED = 4;
    public static final int PICKER_SELECT = 3;
    public static final int RESET_APPLIED = 40;
    public static final int SCREEN_CLOCK = 4;
    public static final int SCREEN_COLORS = 1;
    public static final int SCREEN_ICONS = 2;
    public static final int SCREEN_LAYOUT = 3;
    public static final int SCREEN_SHORTCUTS = 5;
    public static final int SCREEN_UNSPECIFIED = 0;
    public static final int SET_WALLPAPER_ENTRY_POINT_LAUNCHER_WALLPAPER_QUICK_SWITCHER = 3;
    public static final int SET_WALLPAPER_ENTRY_POINT_RESET = 5;
    public static final int SET_WALLPAPER_ENTRY_POINT_RESTORE = 6;
    public static final int SET_WALLPAPER_ENTRY_POINT_ROTATION_WALLPAPER = 4;
    public static final int SET_WALLPAPER_ENTRY_POINT_UNSPECIFIED = 0;
    public static final int SET_WALLPAPER_ENTRY_POINT_WALLPAPER_PREVIEW = 1;
    public static final int SET_WALLPAPER_ENTRY_POINT_WALLPAPER_PREVIEW_SUGGESTED_PHOTOS = 7;
    public static final int SET_WALLPAPER_ENTRY_POINT_WALLPAPER_PREVIEW_SUGGESTED_PHOTOS_CATEGORY_SCREEN = 9;
    public static final int SET_WALLPAPER_ENTRY_POINT_WALLPAPER_PREVIEW_SUGGESTED_PHOTOS_HOME_SCREEN = 8;
    public static final int SET_WALLPAPER_ENTRY_POINT_WALLPAPER_QUICK_SWITCHER = 2;
    public static final int SHAPE_APPLIED = 41;
    public static final int SHORTCUT_APPLIED = 38;
    public static final int SNAPSHOT = 28;
    public static final int THEMED_ICON_APPLIED = 36;
    public static final int THEME_COLOR_APPLIED = 31;
    public static final int WALLPAPER_APPLIED = 7;
    public static final int WALLPAPER_DESTINATION_HOME_AND_LOCK_SCREEN = 3;
    public static final int WALLPAPER_DESTINATION_HOME_SCREEN = 1;
    public static final int WALLPAPER_DESTINATION_LOCK_SCREEN = 2;
    public static final int WALLPAPER_DESTINATION_UNSPECIFIED = 0;
    public static final int WALLPAPER_DOWNLOAD = 9;
    public static final int WALLPAPER_EFFECT_APPLIED = 27;
    public static final int WALLPAPER_EFFECT_FG_DOWNLOAD = 30;
    public static final int WALLPAPER_EFFECT_PROBE = 29;
    public static final int WALLPAPER_EXPLORE = 8;
    public static final int WALLPAPER_OPEN_CATEGORY = 5;
    public static final int WALLPAPER_REMOVE = 10;
    public static final int WALLPAPER_SELECT = 6;
    public StyleEnums() {}
}
