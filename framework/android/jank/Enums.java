package android.jank;

public final class Enums {
    public static final int ACTION_BACK_SYSTEM_ANIMATION = 29;
    public static final int ACTION_CHECK_CREDENTIAL = 4;
    public static final int ACTION_CHECK_CREDENTIAL_UNLOCKED = 5;
    public static final int ACTION_DESKTOP_MODE_ENTER_APP_HANDLE_DRAG = 34;
    public static final int ACTION_DESKTOP_MODE_ENTER_APP_HANDLE_MENU = 35;
    public static final int ACTION_DESKTOP_MODE_EXIT_MODE = 36;
    public static final int ACTION_DESKTOP_MODE_EXIT_MODE_ON_LAST_WINDOW_CLOSE = 37;
    public static final int ACTION_EXPAND_PANEL = 1;
    public static final int ACTION_FACE_WAKE_AND_UNLOCK = 8;
    public static final int ACTION_FINGERPRINT_WAKE_AND_UNLOCK = 3;
    public static final int ACTION_FOLD_TO_AOD = 19;
    public static final int ACTION_KEYGUARD_FACE_UNLOCK_TO_HOME = 32;
    public static final int ACTION_KEYGUARD_FPS_UNLOCK_TO_HOME = 28;
    public static final int ACTION_LOAD_SHARE_SHEET = 17;
    public static final int ACTION_LOCATION_BUTTON_OPEN_SESSION = 40;
    public static final int ACTION_LOCKSCREEN_UNLOCK = 12;
    public static final int ACTION_NOTIFICATIONS_HIDDEN_FOR_MEASURE = 30;
    public static final int ACTION_NOTIFICATIONS_HIDDEN_FOR_MEASURE_WITH_SHADE_OPEN = 31;
    public static final int ACTION_NOTIFICATION_BIG_PICTURE_LOADED = 27;
    public static final int ACTION_REQUEST_IME_HIDDEN = 22;
    public static final int ACTION_REQUEST_IME_SHOWN = 21;
    public static final int ACTION_ROTATE_SCREEN = 7;
    public static final int ACTION_ROTATE_SCREEN_CAMERA_CHECK = 10;
    public static final int ACTION_ROTATE_SCREEN_SENSOR = 11;
    public static final int ACTION_SHADE_WINDOW_DISPLAY_CHANGE = 33;
    public static final int ACTION_SHOW_BACK_ARROW = 16;
    public static final int ACTION_SHOW_SELECTION_TOOLBAR = 18;
    public static final int ACTION_SHOW_VOICE_INTERACTION = 20;
    public static final int ACTION_SMARTSPACE_DOORBELL = 23;
    public static final int ACTION_START_RECENTS_ANIMATION = 9;
    public static final int ACTION_SWITCH_DISPLAY_FOLD = 38;
    public static final int ACTION_SWITCH_DISPLAY_UNFOLD = 14;
    public static final int ACTION_TOGGLE_RECENTS = 2;
    public static final int ACTION_TURN_ON_SCREEN = 6;
    public static final int ACTION_UDFPS_ILLUMINATE = 15;
    public static final int ACTION_UDFPS_OVERLAY_ATTACHED_AFTER_GOING_TO_SLEEP = 39;
    public static final int ACTION_USER_SWITCH = 13;
    public static final int AMBIENT_CUE_COLLAPSE = 148;
    public static final int AMBIENT_CUE_EXPAND = 147;
    public static final int AMBIENT_CUE_HIDE = 146;
    public static final int AMBIENT_CUE_SHOW = 145;
    public static final int APP_UPDATE = 165;
    public static final int BACK_PANEL_ARROW = 89;
    public static final int BIOMETRIC_PROMPT_TRANSITION = 57;
    public static final int BOUNCER_SECURE_LOCK_DEVICE_BIOMETRIC_AUTH_APPEAR = 149;
    public static final int BOUNCER_SECURE_LOCK_DEVICE_BIOMETRIC_AUTH_DISAPPEAR = 150;
    public static final int CLIENT_DRAG_RESIZING = 161;
    public static final int DEFAULT_TASK_TO_TASK_ANIMATION = 128;
    public static final int DESKTOP_MODE_APP_LAUNCH_FROM_ICON = 124;
    public static final int DESKTOP_MODE_APP_LAUNCH_FROM_INTENT = 123;
    public static final int DESKTOP_MODE_CLOSE_TASK = 122;
    public static final int DESKTOP_MODE_CROSS_DISPLAY_DRAG_WINDOW = 174;
    public static final int DESKTOP_MODE_CROSS_DISPLAY_DRAG_WINDOW_RELEASE = 175;
    public static final int DESKTOP_MODE_DESK_SWITCH = 153;
    public static final int DESKTOP_MODE_DESK_TO_FULLSCREEN_SLIDE = 172;
    public static final int DESKTOP_MODE_DRAG_WINDOW = 110;
    public static final int DESKTOP_MODE_DRAG_WINDOW_RELEASE = 171;
    public static final int DESKTOP_MODE_ENTER_APP_HANDLE_DRAG_HOLD = 107;
    public static final int DESKTOP_MODE_ENTER_APP_HANDLE_DRAG_RELEASE = 116;
    public static final int DESKTOP_MODE_ENTER_FROM_OVERVIEW_MENU = 120;
    public static final int DESKTOP_MODE_ENTER_MODE_APP_HANDLE_MENU = 112;
    public static final int DESKTOP_MODE_EXIT_MODE = 108;
    public static final int DESKTOP_MODE_EXIT_MODE_ON_LAST_WINDOW_CLOSE = 117;
    public static final int DESKTOP_MODE_FULLSCREEN_TO_DESK_SLIDE = 180;
    public static final int DESKTOP_MODE_HOME_SCREEN_PEEK = 167;
    public static final int DESKTOP_MODE_HOME_SCREEN_UNPEEK = 168;
    public static final int DESKTOP_MODE_KEYBOARD_QUICK_SWITCH_APP_LAUNCH = 125;
    public static final int DESKTOP_MODE_MAXIMIZE_WINDOW = 104;
    public static final int DESKTOP_MODE_MINIMIZE_WINDOW = 109;
    public static final int DESKTOP_MODE_MOVE_FROM_SPLIT_SCREEN = 144;
    public static final int DESKTOP_MODE_MOVE_TO_SPLIT_SCREEN = 143;
    public static final int DESKTOP_MODE_MOVE_WINDOW_TO_DISPLAY = 129;
    public static final int DESKTOP_MODE_RESIZE_WINDOW = 106;
    public static final int DESKTOP_MODE_SHADE_WINDOW_DISPLAY_CHANGE = 138;
    public static final int DESKTOP_MODE_SNAP_RESIZE = 118;
    public static final int DESKTOP_MODE_TASKBAR_INDICATOR_UPDATE = 162;
    public static final int DESKTOP_MODE_TASKBAR_MULTI_INSTANCE_MENU_OPEN = 163;
    public static final int DESKTOP_MODE_TILE_RESIZING = 140;
    public static final int DESKTOP_MODE_UNMAXIMIZE_WINDOW = 119;
    public static final int DREAM_SWITCHER_EDGE_SWIPE = 173;
    public static final int FHD = 3;
    public static final int FOLD_ANIM = 105;
    public static final int HD = 2;
    public static final int HUN_TO_DUAL_SHADE_NOTIF_OVERLAY = 152;
    public static final int IME_INSETS_ANIMATION = 70;
    public static final int IME_INSETS_HIDE_ANIMATION = 82;
    public static final int IME_INSETS_SHOW_ANIMATION = 81;
    public static final int KEYGUARD_TRANSITION_GONE_TO_DOZING = 160;
    public static final int KEYGUARD_TRANSITION_LOCKSCREEN_TO_DOZING = 159;
    public static final int LAUNCHER_ALL_APPS_SCROLL = 27;
    public static final int LAUNCHER_ALL_APPS_SEARCH_BACK = 95;
    public static final int LAUNCHER_APP_CLOSE_TO_HOME = 10;
    public static final int LAUNCHER_APP_CLOSE_TO_HOME_FALLBACK = 79;
    public static final int LAUNCHER_APP_CLOSE_TO_PIP = 11;
    public static final int LAUNCHER_APP_LAUNCH_FROM_ICON = 9;
    public static final int LAUNCHER_APP_LAUNCH_FROM_RECENTS = 8;
    public static final int LAUNCHER_APP_LAUNCH_FROM_WIDGET = 28;
    public static final int LAUNCHER_APP_SWIPE_TO_RECENTS = 67;
    public static final int LAUNCHER_CLOSE_ALL_APPS_BACK = 90;
    public static final int LAUNCHER_CLOSE_ALL_APPS_SWIPE = 68;
    public static final int LAUNCHER_CLOSE_ALL_APPS_TO_HOME = 69;
    public static final int LAUNCHER_KEYBOARD_QUICK_SWITCH_APP_LAUNCH = 115;
    public static final int LAUNCHER_KEYBOARD_QUICK_SWITCH_CLOSE = 114;
    public static final int LAUNCHER_KEYBOARD_QUICK_SWITCH_OPEN = 113;
    public static final int LAUNCHER_LAUNCH_APP_PAIR_FROM_TASKBAR = 93;
    public static final int LAUNCHER_LAUNCH_APP_PAIR_FROM_WORKSPACE = 92;
    public static final int LAUNCHER_OPEN_ALL_APPS = 26;
    public static final int LAUNCHER_OPEN_SEARCH_RESULT = 72;
    public static final int LAUNCHER_OVERVIEW_CLEAR_ALL = 139;
    public static final int LAUNCHER_OVERVIEW_TASK_DISMISS = 121;
    public static final int LAUNCHER_PRIVATE_SPACE_LOCK = 102;
    public static final int LAUNCHER_PRIVATE_SPACE_UNLOCK = 103;
    public static final int LAUNCHER_QUICK_SWITCH = 12;
    public static final int LAUNCHER_RECENTS_TO_HOME = 166;
    public static final int LAUNCHER_SAVE_APP_PAIR = 94;
    public static final int LAUNCHER_SEARCH_QSB_WEB_SEARCH = 91;
    public static final int LAUNCHER_TASKBAR_ALL_APPS_CLOSE_BACK = 96;
    public static final int LAUNCHER_TASKBAR_ALL_APPS_SEARCH_BACK = 97;
    public static final int LAUNCHER_UNFOLD_ANIM = 84;
    public static final int LAUNCHER_UNLOCK_ENTRANCE_ANIMATION = 64;
    public static final int LAUNCHER_WIDGET_BOTTOM_SHEET_CLOSE_BACK = 100;
    public static final int LAUNCHER_WIDGET_EDU_SHEET_CLOSE_BACK = 101;
    public static final int LAUNCHER_WIDGET_PICKER_APP_EXPAND = 142;
    public static final int LAUNCHER_WIDGET_PICKER_CLOSE_BACK = 98;
    public static final int LAUNCHER_WIDGET_PICKER_OPEN = 141;
    public static final int LAUNCHER_WIDGET_PICKER_SEARCH_BACK = 99;
    public static final int LAUNCHER_WORK_UTILITY_VIEW_EXPAND = 126;
    public static final int LAUNCHER_WORK_UTILITY_VIEW_SHRINK = 127;
    public static final int LOCKSCREEN_CLOCK_MOVE_ANIMATION = 71;
    public static final int LOCKSCREEN_LAUNCH_CAMERA = 52;
    public static final int LOCKSCREEN_OCCLUSION = 65;
    public static final int LOCKSCREEN_PASSWORD_APPEAR = 18;
    public static final int LOCKSCREEN_PASSWORD_DISAPPEAR = 21;
    public static final int LOCKSCREEN_PATTERN_APPEAR = 19;
    public static final int LOCKSCREEN_PATTERN_DISAPPEAR = 22;
    public static final int LOCKSCREEN_PIN_APPEAR = 20;
    public static final int LOCKSCREEN_PIN_DISAPPEAR = 23;
    public static final int LOCKSCREEN_TRANSITION_FROM_AOD = 24;
    public static final int LOCKSCREEN_TRANSITION_TO_AOD = 25;
    public static final int LOCKSCREEN_UNLOCK_ANIMATION = 30;
    public static final int LPP_ASSIST_INVOCATION_EFFECT = 132;
    public static final int NOTIFICATIONS_ANIMATED_ACTION = 131;
    public static final int NOTIFICATION_SHADE_SWIPE = 1;
    public static final int ONE_HANDED_ENTER_TRANSITION = 43;
    public static final int ONE_HANDED_EXIT_TRANSITION = 44;
    public static final int PIP_TRANSITION = 36;
    public static final int PREDICTIVE_BACK_CROSS_ACTIVITY = 85;
    public static final int PREDICTIVE_BACK_CROSS_TASK = 86;
    public static final int PREDICTIVE_BACK_HOME = 87;
    public static final int QHD = 4;
    public static final int RECENTS_SCROLLING = 66;
    public static final int RR_120_HZ = 5;
    public static final int RR_240_HZ = 6;
    public static final int RR_30_HZ = 2;
    public static final int RR_60_HZ = 3;
    public static final int RR_90_HZ = 4;
    public static final int SCREEN_OFF = 41;
    public static final int SCREEN_OFF_SHOW_AOD = 42;
    public static final int SD = 1;
    public static final int SETTINGS_PAGE_SCROLL = 29;
    public static final int SETTINGS_SLIDER = 54;
    public static final int SETTINGS_TOGGLE = 58;
    public static final int SHADE_APP_LAUNCH = 17;
    public static final int SHADE_APP_LAUNCH_FROM_HISTORY_BUTTON = 31;
    public static final int SHADE_APP_LAUNCH_FROM_MEDIA_PLAYER = 32;
    public static final int SHADE_APP_LAUNCH_FROM_QS_TILE = 33;
    public static final int SHADE_APP_LAUNCH_FROM_SETTINGS_BUTTON = 34;
    public static final int SHADE_BLUETOOTH_DETAILS_OPEN = 177;
    public static final int SHADE_CLEAR_ALL = 63;
    public static final int SHADE_DIALOG_OPEN = 59;
    public static final int SHADE_HEADS_UP_APPEAR = 13;
    public static final int SHADE_HEADS_UP_DISAPPEAR = 14;
    public static final int SHADE_INTERNET_DETAILS_OPEN = 176;
    public static final int SHADE_NOTIFICATION_ADD = 15;
    public static final int SHADE_NOTIFICATION_REMOVE = 16;
    public static final int SHADE_QS_EXPAND_COLLAPSE = 6;
    public static final int SHADE_QS_SCROLL_SWIPE = 7;
    public static final int SHADE_ROW_EXPAND = 4;
    public static final int SHADE_ROW_SWIPE = 5;
    public static final int SHADE_SCROLL_FLING = 3;
    public static final int SHADE_TILE_DETAILS_OPEN = 178;
    public static final int SPLASHSCREEN_AVD = 39;
    public static final int SPLASHSCREEN_EXIT_ANIM = 40;
    public static final int SPLIT_SCREEN_DOUBLE_TAP_DIVIDER = 83;
    public static final int SPLIT_SCREEN_ENTER = 50;
    public static final int SPLIT_SCREEN_EXIT = 51;
    public static final int SPLIT_SCREEN_RESIZE = 53;
    public static final int STATUS_BAR_APP_LAUNCH_FROM_CALL_CHIP = 35;
    public static final int STATUS_BAR_APP_RETURN_TO_CALL_CHIP = 130;
    public static final int STATUS_BAR_APP_RETURN_TO_ONGOING_CHIP = 151;
    public static final int STATUS_BAR_LAUNCH_DIALOG_FROM_CHIP = 111;
    public static final int SUW_LOADING_SCREEN_FOR_STATUS = 49;
    public static final int SUW_LOADING_TO_NEXT_FLOW = 48;
    public static final int SUW_LOADING_TO_SHOW_INFO_WITH_ACTIONS = 46;
    public static final int SUW_SHOW_FUNCTION_SCREEN_WITH_ACTIONS = 47;
    public static final int TAKE_SCREENSHOT = 55;
    public static final int TASKBAR_COLLAPSE = 62;
    public static final int TASKBAR_EXPAND = 61;
    public static final int TASKBAR_ICON_APPEAR = 164;
    public static final int UNFOLD_ANIM = 45;
    public static final int UNKNOWN_ACTION = 0;
    public static final int UNKNOWN_INTERACTION = 0;
    public static final int UNKNOWN_REFRESH_RATE = 0;
    public static final int UNKNOWN_RESOLUTION = 0;
    public static final int UNLOCK_AUTH_RIPPLE = 179;
    public static final int USER_DIALOG_OPEN = 60;
    public static final int USER_SWITCH = 38;
    public static final int VARIABLE_REFRESH_RATE = 1;
    public static final int VOLUME_CONTROL = 56;
    public static final int WALLPAPER_TRANSITION = 37;
    public static final int WEAR_ALERT_NOTIFICATION_DIALOG = 158;
    public static final int WEAR_APP_LAUNCH_TRANSITION = 170;
    public static final int WEAR_BOUNCER_EXIT_ANIMATION = 154;
    public static final int WEAR_BOUNCER_PATTERN_ENTRY = 157;
    public static final int WEAR_BOUNCER_PIN_DOT_ADDED = 156;
    public static final int WEAR_BOUNCER_PIN_PAD_ENTRY = 155;
    public static final int WEAR_CAROUSEL_FLING_JANK = 134;
    public static final int WEAR_CAROUSEL_SCROLL_JANK = 133;
    public static final int WEAR_CAROUSEL_SWIPE_JANK = 135;
    public static final int WEAR_NOTIFICATION_TRAY_OPEN = 137;
    public static final int WEAR_ORIGIN_TRANSITION_OPEN = 169;
    public static final int WEAR_QSS_TRAY_OPEN = 136;
    public Enums() {}
}
