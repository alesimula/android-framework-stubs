package android.hardware.biometrics;

public final class BiometricsProtoEnums {
    public static final int ACTION_AUTHENTICATE = 2;
    public static final int ACTION_ENROLL = 1;
    public static final int ACTION_ENUMERATE = 3;
    public static final int ACTION_REMOVE = 4;
    public static final int ACTION_UNKNOWN = 0;
    public static final int AUTH_TIMEOUT_REASON_NO_PRESENCE_DETECTED_NO_FACE_DETECTED_LOW_LIGHT = 6;
    public static final int AUTH_TIMEOUT_REASON_NO_PRESENCE_DETECTED_NO_FACE_DETECTED_NO_LIGHT = 7;
    public static final int AUTH_TIMEOUT_REASON_NO_PRESENCE_DETECTED_NO_FACE_DETECTED_SUFFICIENT_LIGHT = 5;
    public static final int AUTH_TIMEOUT_REASON_PRESENCE_DETECTED_NO_FACE_DETECTED_LOW_LIGHT = 3;
    public static final int AUTH_TIMEOUT_REASON_PRESENCE_DETECTED_NO_FACE_DETECTED_NO_LIGHT = 4;
    public static final int AUTH_TIMEOUT_REASON_PRESENCE_DETECTED_NO_FACE_DETECTED_SUFFICIENT_LIGHT = 2;
    public static final int AUTH_TIMEOUT_REASON_STREAM_LIMIT_REACHED = 1;
    public static final int AUTH_TIMEOUT_REASON_UNKNOWN = 0;
    public static final int CLIENT_BIOMETRIC_PROMPT = 2;
    public static final int CLIENT_FINGERPRINT_MANAGER = 3;
    public static final int CLIENT_KEYGUARD = 1;
    public static final int CLIENT_UNKNOWN = 0;
    public static final int DETAILS_FACE_ALTERNATE_BIOMETRIC_BOUNCER_SHOWN = 4;
    public static final int DETAILS_FACE_ASSISTANT_VISIBLE = 3;
    public static final int DETAILS_FACE_NOTIFICATION_PANEL_CLICKED = 5;
    public static final int DETAILS_FACE_OCCLUDING_APP_REQUESTED = 6;
    public static final int DETAILS_FACE_PICK_UP_GESTURE_TRIGGERED = 7;
    public static final int DETAILS_FACE_PRIMARY_BOUNCER_SHOWN = 2;
    public static final int DETAILS_FACE_QS_EXPANDED = 8;
    public static final int DETAILS_FACE_STARTED_WAKING_UP = 1;
    public static final int DETAILS_FACE_SWIPE_UP_ON_BOUNCER = 9;
    public static final int DETAILS_FACE_UDFPS_POINTER_DOWN = 10;
    public static final int DETAILS_UNKNOWN = 0;
    public static final int EMUMERATION_RESULT_TIMEOUT = 5;
    public static final int ENROLLMENT_SOURCE_FRR_NOTIFICATION = 3;
    public static final int ENROLLMENT_SOURCE_SETTINGS = 2;
    public static final int ENROLLMENT_SOURCE_SUW = 1;
    public static final int ENROLLMENT_SOURCE_UNKNOWN = 0;
    public static final int ENUMERATION_RESULT_DANGLING_BOTH = 4;
    public static final int ENUMERATION_RESULT_DANGLING_FRAMEWORK = 3;
    public static final int ENUMERATION_RESULT_DANGLING_HAL = 2;
    public static final int ENUMERATION_RESULT_OK = 1;
    public static final int ENUMERATION_RESULT_UNKNOWN = 0;
    public static final int FOLD_CLOSED = 2;
    public static final int FOLD_HALF_OPEN = 3;
    public static final int FOLD_OPEN = 1;
    public static final int FOLD_UNKNOWN = 0;
    public static final int FRR_NOTIFICATION_ACTION_CLICKED = 2;
    public static final int FRR_NOTIFICATION_ACTION_DISMISSED = 3;
    public static final int FRR_NOTIFICATION_ACTION_SHOWN = 1;
    public static final int FRR_NOTIFICATION_ACTION_UNKNOWN = 0;
    public static final int ISSUE_CANCEL_TIMED_OUT = 4;
    public static final int ISSUE_FINGERPRINTS_LOE = 5;
    public static final int ISSUE_HAL_DEATH = 1;
    public static final int ISSUE_UNKNOWN = 0;
    public static final int ISSUE_UNKNOWN_TEMPLATE_ENROLLED_FRAMEWORK = 2;
    public static final int ISSUE_UNKNOWN_TEMPLATE_ENROLLED_HAL = 3;
    public static final int MODALITY_FACE = 4;
    public static final int MODALITY_FINGERPRINT = 1;
    public static final int MODALITY_IRIS = 2;
    public static final int MODALITY_UNKNOWN = 0;
    public static final int ORIENTATION_0 = 1;
    public static final int ORIENTATION_180 = 3;
    public static final int ORIENTATION_270 = 4;
    public static final int ORIENTATION_90 = 2;
    public static final int ORIENTATION_UNKNOWN = 0;
    public static final int SENSOR_FACE_IR = 7;
    public static final int SENSOR_FACE_RGB = 6;
    public static final int SENSOR_FP_HOME_BUTTON = 5;
    public static final int SENSOR_FP_POWER_BUTTON = 4;
    public static final int SENSOR_FP_REAR = 1;
    public static final int SENSOR_FP_STANDALONE = 8;
    public static final int SENSOR_FP_UDFPS_OPTICAL = 3;
    public static final int SENSOR_FP_UDFPS_ULTRASONIC = 2;
    public static final int SENSOR_UNKNOWN = 0;
    public static final int SESSION_TYPE_BIOMETRIC_PROMPT = 2;
    public static final int SESSION_TYPE_KEYGUARD_ENTRY = 1;
    public static final int SESSION_TYPE_UNKNOWN = 0;
    public static final int STRENGTH_CONVENIENCE = 1;
    public static final int STRENGTH_STRONG = 3;
    public static final int STRENGTH_UNKNOWN = 0;
    public static final int STRENGTH_WEAK = 2;
    public static final int TOUCH_TYPE_CANCEL = 3;
    public static final int TOUCH_TYPE_DOWN = 1;
    public static final int TOUCH_TYPE_UNCHANGED = 0;
    public static final int TOUCH_TYPE_UP = 2;
    public static final int TUSCANY_AUTH_DECISION_ACCEPTED = 1;
    public static final int TUSCANY_AUTH_DECISION_CANCELLED = 4;
    public static final int TUSCANY_AUTH_DECISION_ERROR = 6;
    public static final int TUSCANY_AUTH_DECISION_FACE_DETECT = 5;
    public static final int TUSCANY_AUTH_DECISION_REJECTED = 2;
    public static final int TUSCANY_AUTH_DECISION_TIMEOUT = 3;
    public static final int TUSCANY_AUTH_DECISION_UNKNOWN = 0;
    public static final int TUSCANY_CANCEL_REASON_BINDER_ERROR = 5;
    public static final int TUSCANY_CANCEL_REASON_FPS_AUTH = 3;
    public static final int TUSCANY_CANCEL_REASON_FRAMEWORK_ERROR = 4;
    public static final int TUSCANY_CANCEL_REASON_PASSCODE_AUTH = 2;
    public static final int TUSCANY_CANCEL_REASON_UNKNOWN = 0;
    public static final int TUSCANY_CANCEL_REASON_USER_CANCELLED = 1;
    public static final int TUSCANY_ERROR_REASON_HARD_ERROR_DURING_AUTH = 9;
    public static final int TUSCANY_ERROR_REASON_UNABLE_CAPTURE_IMAGE = 5;
    public static final int TUSCANY_ERROR_REASON_UNABLE_CREATE_AUTH_CONTROLLER = 4;
    public static final int TUSCANY_ERROR_REASON_UNABLE_GET_PRIMARY_INPUT = 6;
    public static final int TUSCANY_ERROR_REASON_UNABLE_GET_RAW_DATA = 8;
    public static final int TUSCANY_ERROR_REASON_UNABLE_GET_SECONDARY_INPUT = 7;
    public static final int TUSCANY_ERROR_REASON_UNABLE_GET_WAKELOCK = 2;
    public static final int TUSCANY_ERROR_REASON_UNABLE_OPEN_CAMERA = 3;
    public static final int TUSCANY_ERROR_REASON_UNKNOWN = 0;
    public static final int TUSCANY_ERROR_REASON_USER_NOT_ENROLLED = 1;
    public static final int UNENROLL_REASON_DANGLING_FRAMEWORK = 2;
    public static final int UNENROLL_REASON_DANGLING_HAL = 1;
    public static final int UNENROLL_REASON_UNKNOWN = 0;
    public static final int UNENROLL_REASON_USER_REQUEST = 3;
    public static final int WAKE_REASON_BIOMETRIC = 9;
    public static final int WAKE_REASON_DISPLAY_GROUP_ADDED = 6;
    public static final int WAKE_REASON_GESTURE = 2;
    public static final int WAKE_REASON_LID = 5;
    public static final int WAKE_REASON_LIFT = 8;
    public static final int WAKE_REASON_POWER_BUTTON = 1;
    public static final int WAKE_REASON_TAP = 7;
    public static final int WAKE_REASON_UNKNOWN = 0;
    public static final int WAKE_REASON_WAKE_KEY = 3;
    public static final int WAKE_REASON_WAKE_MOTION = 4;
    public BiometricsProtoEnums() {}
}
