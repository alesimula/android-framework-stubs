package com.android.internal.hidden_from_bootclasspath.android.view.contentprotection.flags;

public final class Flags {
    public static final java.lang.String FLAG_CREATE_ACCESSIBILITY_OVERLAY_APP_OP_ENABLED = "android.view.contentprotection.flags.create_accessibility_overlay_app_op_enabled";
    public static final java.lang.String FLAG_EXPORTED_SETTINGS_ACTIVITY_ENABLED = "android.view.contentprotection.flags.exported_settings_activity_enabled";
    public static final java.lang.String FLAG_MANAGE_DEVICE_POLICY_ENABLED = "android.view.contentprotection.flags.manage_device_policy_enabled";
    public static final java.lang.String FLAG_RAPID_CLEAR_NOTIFICATIONS_BY_LISTENER_APP_OP_ENABLED = "android.view.contentprotection.flags.rapid_clear_notifications_by_listener_app_op_enabled";
    public Flags() {}
    public static boolean createAccessibilityOverlayAppOpEnabled() { return false; }
    public static boolean exportedSettingsActivityEnabled() { return false; }
    public static boolean manageDevicePolicyEnabled() { return false; }
    public static boolean rapidClearNotificationsByListenerAppOpEnabled() { return false; }
}
